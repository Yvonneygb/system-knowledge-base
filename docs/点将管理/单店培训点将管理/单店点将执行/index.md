...
- **前端路由**：`/general/singleStoreGeneral/singleStoreGeneralExecute/list`
- **API**：`mlt/trainApply/page`（查询条件增加执行状态过滤）
- **Entity**：`TrainApply`
- **查询条件**：申请单号、讲师姓名、经销商名称、门店名称、执行状态、培训时间范围
- **列表字段**：申请单号、培训主题、讲师姓名、经销商名称、门店名称、培训时间、执行状态、完成时间、创建时间
- **操作按钮**：查看详情、更新进度、完成执行
...
- **API**：`mlt/trainApply/updateProgress`
...
<KbCard num="1" title="train_apply（单店培训点将申请表-执行相关字段）">
