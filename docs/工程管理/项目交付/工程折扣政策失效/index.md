...
| 下游单据/系统 | 影响说明 |
|-------------|---------|
| 工程折扣政策行 | 审批通过后，关联的政策行生效状态更新为已失效(VALID_STAT=3) |
| 工程折扣政策头 | 当该政策下所有行均失效后，政策头有效状态更新为已失效(VALID=3) |
| CRM系统 | 通用类型(suitableType=normal)不推送CRM；专项类型(suitableType=special)审批通过后推送CRM执行失效 |
| OA系统 | 保存并提交时推送OA审批，携带头信息和明细行数据 |
