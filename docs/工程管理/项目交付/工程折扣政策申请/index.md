...
| 下游系统/模块 | 影响内容 | 说明 |
|---|---|---|
| 工程要货订单 | 要货订单价格计算 | 工程要货订单选择折扣政策时，依据折扣政策的产品明细和阶梯折扣率/特价计算折后单价 |
| EBS接口/产品资料 | 产品扩展资料更新 | 流程提交时从EBS接口获取并更新产品的月平均动销数量、库存消化周期、库存数量、生命状态、新品标识等 |
| CRM系统 | CRM折扣率校验 | 非样品/长库龄业务类型时，流程提交校验CRM折扣率(如果启用折扣管控且客户需校验) |
| 工程要货订单 | 经销商可使用政策 | 政策审批通过后，经销商在要货订单中可选择该折扣政策 |
