...
```text
用户选择查询条件(法人/交易公司/经销商/款项类型) → 调用EBS接口(ArrowEbsSdkService)实时查询 → 返回经销商余额明细数据 → 展示列表
```
...
|---------|---------|---------|------------|
| EBS系统 | 接口依赖 | 通过ArrowEbsSdkService实时查询经销商余额数据 | EBS接口可用且网络连通 |
| 客户主文件 | 数据依赖 | CUSTOMER_ORG表提供经销商基础信息 | 客户数据已维护 |
