...
- `kl-tip` 用于承载**业务层面的原则、结论、约束**——例如"审核必须满足最低真实性标准""两套业务各自独立、互不影响"。让不了解该业务的人一眼看懂背后的业务规则与边界。
- **严禁在 `kl-tip`（以及整个【业务介绍】）中出现代码级实现细节**：如后端编码规则（`isHome=2`、`AE.JZ_INVOICE_VERIFER_NO`）、工作流标识（`INVOICE_JZHXFPSC_AW`）、接口名、字段名、表名、常量、模块标识等。这些应归入页面的【重点逻辑 key-logic】，而非业务介绍。
...
3. **禁止代码级细节**：业务介绍只讲业务是什么、为什么、怎么流转。编码规则（`isHome=2`、单据编号规则）、工作流标识、接口/字段/表名、常量、模块代码一律**不出现**；这些归入页面【重点逻辑 key-logic】。
4. **图文结合**：要点用图标卡，流程用步骤条，标准用表格，演进用时间轴。
5. **克制篇幅**：单功能页卡片 ≤3，正文点到为止，细节交底给「重点逻辑」。
6. **数据准确**：比例、金额、时限等写具体数值（如 85%、¥10,000、120 天），不写"较高/较短"。
7. **层次清晰**：标题用 `biz-kl-hdr`（标签+h2+副标题），正文用 `h4/h5` 小标题，禁用裸大段 `<p>`。
