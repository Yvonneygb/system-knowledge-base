...
> **🔴 强制标准（所有业务流程图通用）**：上述 `tab-pad → bf-truth-flow → (bf-main-title / bf-main-sub / bf-fc-svg-wrap > bf-fc-svg / bf-fc-legend)` 是业务流程图区块的**唯一外层结构**。任何菜单页 `<div id="biz-flow">` 内都必须严格按此嵌套，**禁止**用 `kl-wrap` / 裸 `KbCard` 直接包 SVG，也禁止把 `bf-fc-svg` / `bf-fc-legend` 放到 `bf-fc-svg-wrap` 之外。
...
### 3.3 T 形垂直分叉线（判断节点 → 两个并列分支卡片，强制标准）

> **🔴 标准：判断菱形下方若分裂出两个并列分支卡片，必须用「T 形垂直连接」——从菱形底部先垂直向下一小段 → 水平分叉 → 各自再垂直向下，箭头落在每个分支卡片的【顶部中心】。禁止用水平箭头从卡片侧面戳入（旧 Y 形，已废止）。**

适用于「家装类型（直销/经销）」「是否符合条件」等产生两个并列卡片的判断节点。

```xml
<!-- ① 菱形底（底顶点 y=289）→ 垂直下到分叉点（y=300） -->
<line x1="235" y1="289" x2="235" y2="300" stroke="#16A34A" stroke-width="2"/>

<!-- ② 水平分叉：从中心 x=235 向左右两个分支卡片的中心 x 拉平 -->
...
<!-- ③ 垂直进入两个分支卡片的【顶部中心】，箭头朝下 -->
<!-- 分支卡1：x=20,width=200 → 中心 x=120；卡顶 y=314 -->
<line x1="120" y1="300" x2="120" y2="314" stroke="#16A34A" stroke-width="2" marker-end="url(#arr-green)"/>
<!-- 分支卡2：x=250,width=220 → 中心 x=360；卡顶 y=314 -->
...
| 规则 | 说明 |
|------|------|
| 卡片中心对齐分叉端点 | 每个分支卡片的 x 中心 = 对应水平分叉线的端点 x（左端点→左卡，右端点→右卡），即 `card_center_x = card_left + card_width/2` |
| 垂直进入顶部中心 | 从分叉点垂直向下，箭头精准落在卡片上边缘中点（x=card_center_x, y=card_top） |
| 卡片间距 | 两个分支卡片左右之间留 30~40px 间隙，避免拥挤；两张卡片顶部 y 保持一致 |
| 三段垂直间距 | 菱形底→分叉点 8~12px；分叉点→卡片顶 ≥14px（放箭头）；整体约 22~26px，符合 §5.1 纵向间距原则 |
| 禁止侧面水平箭头 | 不得用水平 `<line>` 从卡片左侧/右侧戳入（旧 Y 形，已废止） |
...
```
    ...
    ┌────┴────┐
    ↓         ↓              ← 垂直进入卡片【顶部中心】
  [直销分支]  [经销分支]        ← 卡片顶 y=314，中心 x=120 / 360
```

> 单条「拒绝/返回」分支（不是两个并列卡片）仍可用 §3.4.2 的 L 形/水平线连到卡片侧面，不受本条限制；本条只约束「一个菱形 → 两个并列卡片」的情形。
...
<!-- ❌ 水平箭头从卡片侧面戳入（已废止，须改 T 形垂直连接，见 §3.3） -->
...
>
> 判断菱形→分叉点→分支卡片顶的**三段式垂直连接（T 形，见 §3.3）**同样遵循本原则：菱形底→分叉点 8~12px、分叉点→卡片顶 **≥14px**（放箭头），整体约 22~26px，确保箭头不被卡片遮挡、也不得因过近而看不见。
...
14. **双分支连接用 T 形垂直（见 §3.3）**：一个判断菱形分裂出两个并列分支卡片时，必须从菱形底垂直下 → 水平分叉 → 再垂直下，箭头进入各卡片【顶部中心】；禁止用水平箭头从卡片侧面戳入。
